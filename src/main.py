...
def get_auth_token(auth_url: str, service_url: str, image: str) -> str:
    r = requests.get("{}/token?service={}&scope=repository:library/{}:pull".format(auth_url, service_url, image))
    try:
        data = r.json()
...
def get_image_data(registry_url: str, token: str, image: str, digest: str = "latest") -> dict[str, any]:
    headers = {
    ...
    r = requests.get('{}/v2/library/{}/manifests/{}'.format(registry_url, image, digest), headers=headers)
    return r.json() 


def filter_for_arch_and_os(arch: str, os: str) -> Callable[[dict[str, any]], bool]:

    def fn(d: dict[str, any]):
        return d['platform'] is not None and d['platform']['architecture'] == arch and d['platform']['os'] == os
    return fn


def get_digest_from_manifests(arch: str, os: str, img_data: dict[str, any]) -> str:
    found_manifests = filter(filter_for_arch_and_os(arch, os), image_data['manifests'])
...
def download_layer(registry_url: str,
        auth_token: str,
        image: str, layer_digest: str, filename: str) -> None:
    headers = {
...
if __name__ == "__main__":
    ...
    for (i, l) in enumerate(digests_for_layers):
        download_layer(registry_url, token, image, l, 'layer_{}.tar.gz'.format(i))
    print(digests_for_layers)
